...
int main(int argc, char ** argv) {
  ...
  for (auto i = 0; i < queries.size(); i++) {
    unordered_multimap<TrieNode*, pair<int, int>> agg; // the previous matches  <row, ed>
    unordered_map<TrieNode*, pair<int, int>> matches;    // the current matches  <col, ed>
    agg.insert(make_pair(trie->root, make_pair(0, 0)));          // initialize agg with  <root, 0, 0>
    ...
    for (auto j = 1; j <= queries[i].length(); j++) {
      ...
      for (auto it = agg.begin(); it != agg.end(); ++it) {   // foreach <node, list<row, ed>>
        for (int count = 0; count <= tau; ++count) { // roll down count levels
          ...
          while (vit != vec.end() && (*vit)->id <= it->first->last) {
            ...
            int themin = min(tau + 1, it->second.second + max(j - 1 - it->second.first, count));
            if (themin <= tau) {
      ...
      for (auto mit = matches.begin(); mit != matches.end(); mit++)
        agg.insert(make_pair(mit->first, mit->second));

      // remove old matches
      for (auto ait = agg.begin(); ait != agg.end(); ) {
        if (j - ait->second.first + ait->second.second > tau) {
          ait = agg.erase(ait);
        } else {
          ait++;
        }
