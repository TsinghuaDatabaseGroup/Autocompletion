...
void work(const std::string &str, std::multiset<std::pair<int, TrieNode*>>::iterator &iter,
    ...
    while (!out_range.empty()) {
        ...
        int x, y;
        getpos(match.second.first, match.second.second.first->node->depth, match.second.second.second, x, y);
    ...
    while (!M1.empty()) {
        ...
        int cx, cy, x = -1, y = -1, ux = -1, bx = -1, ry = -1;
        std::vector<Match*> M;
        ...
        for (int k = M.size() - 1; k >= 0; --k) {
            ...
            if (x > ux && x < bx && y < ry) {
                if (y + match->md > ry) pos += ry - y;
                else pos += match->md + bx - x;
            }
            ...
            ux = match->i + 1;
            bx = match->i + match->md + 1;
            ry = mnode->node->depth + match->md + 1;
            ++match->md;
        }
